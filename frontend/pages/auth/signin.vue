...
const login = async (e) => {
    e.preventDefault()
    const response = await signIn('credentials', { callbackUrl: '/protected/globally', redirect: true, username: credentials.username, password: credentials.password })
};
</script>
    ...
    <section class="grid grid-cols-2 wrapper my-6 mx-auto items-center justify-center">
        ...
        <div class="bg-gray-100 flex mx-auto rounded-2xl ease-in-out duration-300 shadow-xl max-w-[60rem] p-5 items-center">
            ...
            <div class=" px-8 md:px-16">
                ...
                <p class="text-xs mt-4 text-[#002D74]">Strapi users can login with their username (or e-mail) and password.
                </p>

                <form class="flex flex-col gap-4">
                    <input name="csrfToken" type="hidden" v-model="credentials.csrfToken" />
                    <input v-model="credentials.username" class="p-2 mt-8 rounded-xl border" type="text" name="email"
                        placeholder="Email">
                    <div class="relative">
                        <input v-model="credentials.password" class="p-2 rounded-xl border w-full" type="password"
